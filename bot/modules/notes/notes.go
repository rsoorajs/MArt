...
func cmdGet(bot ext.Bot, u *gotgbot.Update, args []string) error {
	if len(args) >= 2 && strings.ToLower(args[1]) == "noformat" {
		...
		_, err := u.EditedMessage.ReplyText("Get mad")
		return err
	}
}
...
func hashGet(bot ext.Bot, u *gotgbot.Update) error {
	...
	if len(fstWord) <= 0 {
		_, err := msg.ReplyText("Something went wrong! Contact @keralasbots to figure out what.")
		return err
	}
	noHash := strings.ToLower(fstWord[1:])
...
func save(_ ext.Bot, u *gotgbot.Update) error {
	...
	go sql.AddNoteToDb(strconv.Itoa(chatId), fnote, text, dataType, btns, content)
